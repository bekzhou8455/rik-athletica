...
// === Look up the referrer's row by slug (for post-conversion notification) ===
export async function findReferrerBySlug(slug) {
  if (!slug) return null;
  const result = await sql`
    SELECT id, email, first_name FROM audits
    WHERE slug = ${slug} AND status = 'delivered'
    LIMIT 1;
  `;
  return result.rows[0] ?? null;
...
// === Count of audits referred by a specific slug (referrer dashboard) ===
export async function countReferralsByslug(slug) {
  if (!slug) return 0;
  const result = await sql`
    SELECT COUNT(*) AS n FROM audits
    WHERE referred_by_slug = ${slug};
  `;

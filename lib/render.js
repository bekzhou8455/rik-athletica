...
.share-social-row { margin-top: 18px; padding-top: 18px; border-top: 1px dashed var(--rule); width: 100%; max-width: 340px; }
.share-social-row .share-social-label { display: block; font-size: 10px; letter-spacing: 0.18em; text-transform: uppercase; color: var(--ink-soft); font-weight: 600; margin-bottom: 12px; }
.share-icons { display: flex; gap: 10px; flex-wrap: wrap; }
.share-icon { width: 42px; height: 42px; display: inline-flex; align-items: center; justify-content: center; border-radius: 999px; text-decoration: none; transition: transform 150ms ease, box-shadow 150ms ease; box-shadow: 0 2px 6px rgba(14,14,14,.08); }
.share-icon:hover { transform: translateY(-2px); box-shadow: 0 4px 12px rgba(14,14,14,.16); }
.share-icon svg { width: 18px; height: 18px; }
.share-icon.wa { background: #25D366; color: #fff; }
.share-icon.sms { background: #34C759; color: #fff; }
.share-icon.email { background: var(--ink); color: #fff; }
.share-icon.x { background: #000; color: #fff; }
.share-icon.ig { background: linear-gradient(135deg, #833AB4 0%, #FD1D1D 55%, #FCB045 100%); color: #fff; }
.share-icon.tiktok { background: #000; color: #fff; }
.share-icon.copy { background: var(--bg); color: var(--ink); border: 1px dashed var(--rule); }
.share-icon.copy:hover { background: #fff; }
.share-icon[type="button"] { border: none; cursor: pointer; padding: 0; font-family: inherit; }
        ...
        <div>
          ...
          <div class="share-actions">
            ...
            <div class="share-social-row">
              <span class="share-social-label">Or send directly to one app</span>
              <div class="share-icons">
                <a class="share-icon wa" id="share-wa" href="#" target="_blank" rel="noopener" aria-label="Share via WhatsApp" data-share-platform="whatsapp">
                ...
                <a class="share-icon sms" id="share-sms" href="#" aria-label="Share via Messages" data-share-platform="sms">
                  <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M12 2C6.48 2 2 6.06 2 11c0 2.43 1.07 4.63 2.79 6.21L4 22l5.06-1.92c.92.24 1.91.42 2.94.42 5.52 0 10-4.06 10-9s-4.48-9-10-9zm-3 10H7v-2h2v2zm4 0h-2v-2h2v2zm4 0h-2v-2h2v2z"/></svg>
                </a>
                <a class="share-icon email" id="share-email" href="#" aria-label="Share via Email" data-share-platform="email">
                ...
                <a class="share-icon x" id="share-x" href="#" target="_blank" rel="noopener" aria-label="Share on X" data-share-platform="x">
                  <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
                </a>
                <button class="share-icon ig" type="button" id="share-ig" aria-label="Copy message + open Instagram" data-share-platform="instagram">
                ...
                <button class="share-icon tiktok" type="button" id="share-tiktok" aria-label="Copy message + open TikTok" data-share-platform="tiktok">
...
(function() {
  ...
  const xMessage = 'Just had my race fueling audited — worth 90 sec before any race:';
  ...
  function buildEmailBody(recipient) {
    const name = recipient || 'there';
    return 'Hi ' + name + ',\\n\\nGot my fueling audited and figured You might want a look before Your next race. It\\'s a 90-second read:\\n\\n' + auditUrl + '\\n\\n— ' + senderFirstName;
  }

  // === Share URL builders (one per platform) ===
  const shareLinks = {
    wa:    document.getElementById('share-wa'),
    sms:   document.getElementById('share-sms'),
    email: document.getElementById('share-email'),
    x:     document.getElementById('share-x'),
  };

  function updateShareUrls(recipient) {
    const msg = buildMessage(recipient);
    const emailBody = buildEmailBody(recipient);
    const emailSubject = 'Fueling audit — worth a look?';
    if (shareLinks.wa)    shareLinks.wa.href    = 'https://wa.me/?text=' + encodeURIComponent(msg);
    if (shareLinks.sms)   shareLinks.sms.href   = 'sms:?&body=' + encodeURIComponent(msg);
    if (shareLinks.email) shareLinks.email.href = 'mailto:?subject=' + encodeURIComponent(emailSubject) + '&body=' + encodeURIComponent(emailBody);
    if (shareLinks.x)     shareLinks.x.href     = 'https://twitter.com/intent/tweet?text=' + encodeURIComponent(xMessage) + '&url=' + encodeURIComponent(auditUrl);
  }
  ...
  // Initialize share URLs on page load
  updateShareUrls('');
  ...
  // Track per-platform share clicks (URL-prefill platforms)
  Object.entries(shareLinks).forEach(([platform, el]) => {
    if (!el) return;
    el.addEventListener('click', function() {
      const recipient = input.value.trim().replace(/[<>]/g, '').slice(0, 24);
      if (window.gtag) gtag('event', 'audit_share_platform', { platform: platform, recipient_named: !!recipient });
    });
  });

  // === IG / TikTok — copy-and-open pattern ===
  // Neither platform supports URL-prefill share intents. So we copy the
  // message to clipboard, show a contextual toast, and open the app/site
  // in a new tab so the user can paste into a story/post/DM.
  const copyAndOpen = [
    { id: 'share-ig',     platform: 'instagram', url: 'https://www.instagram.com/', label: 'Instagram' },
    { id: 'share-tiktok', platform: 'tiktok',    url: 'https://www.tiktok.com/',    label: 'TikTok' },
  ];
  ...
  copyAndOpen.forEach(function(spec) {
    ...
    btn.addEventListener('click', async function(e) {
      ...
      // Contextual toast
      toast.textContent = 'Copied — paste into ' + spec.label;
      toast.classList.add('show');
      setTimeout(function() {
        toast.classList.remove('show');
        // Restore default text after fade-out
        setTimeout(function() { toast.textContent = 'Copied to clipboard ✓'; }, 250);
      }, 2400);
      // Open platform in a new tab so user can paste
      window.open(spec.url, '_blank', 'noopener');
      // Track
      if (window.gtag) gtag('event', 'audit_share_platform', { platform: spec.platform, recipient_named: !!recipient });
    });
  });
})();
</script>

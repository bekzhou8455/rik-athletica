...
export async function alertNewDraft({ audit, engine, drafts, answers }) {
  ...
  const referredBy = answers?.referredBy?.trim() || null;
  const url = adminUrl(`/admin/audit-queue.html?id=${audit.id}`);
  const referredByBlock = referredBy ? `
    <p style="font-family:system-ui;font-size:13px;background:#fff8e6;border-left:3px solid #f0b400;padding:10px 14px;margin:0 0 16px;border-radius:4px">
      <strong>Referred by:</strong> ${escapeHtml(referredBy)}
    </p>` : '';
  const subjectSuffix = referredBy ? ` · ref: ${referredBy.slice(0, 30)}` : '';
  const html = `
  ...
  `;
  await sendAlert({ subject: `🟢 New audit drafted: ${audit.first_name} (${distance}, ${weeksOut}w)${subjectSuffix}`, html });
}
...
export async function emailAuditToAthlete({ to, firstName, slug, pdfUrl, tierLabel }) {
  ...
  const html = `
    <div style="font-family:'Helvetica Neue',Arial,sans-serif;color:#0a0a0a;max-width:560px;line-height:1.65;padding:32px 24px">
      ...
      <div style="font-size:11px;font-weight:700;letter-spacing:2.5px;text-transform:uppercase;color:#0E0E0E;margin-bottom:8px">RIK Athletica · Personal Audit</div>
      ...
      <p style="font-size:16px;color:#1a1a1a;margin:0 0 8px">I read your responses and put together a personalized fueling overview showing exactly where the minutes are leaking — and the next step that closes the gap.</p>
      ...
      <table role="presentation" cellpadding="0" cellspacing="0" border="0" style="margin:0 auto 32px">
        <tr>
          <td style="background:#0E0E0E;border-radius:36px;text-align:center">
        ...
        <tr>
          <td style="padding-top:8px;text-align:center;font-size:12px;color:#888;letter-spacing:0.4px">personalized fueling overview · printable PDF inside</td>
        </tr>
      </table>
...
export async function emailE0Confirmation({ to, firstName }) {
  const html = `
    <div style="font-family:system-ui;color:#0a0a0a;max-width:560px;line-height:1.6">
      ...
      <p>Got your audit submission. I personally review every one — within <strong>2 business days</strong>, your personalized audit page lands in your inbox.</p>
  ...
  await t.sendMail({
    ...
    html,
    text: `Hi ${firstName},\n\nGot your submission. I review every audit personally — within 2 business days you'll have your personalized audit page ready.\n\nReply if you have any questions.\n— Bek`,
  });
}

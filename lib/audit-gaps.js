...
// === Brand → carb-transport classification (for personalized dual-transport tactic) ===
// Brands with clear 1:0.8 glucose:fructose dual-transport products
const DUAL_TRANSPORT_BRANDS = {
  maurten:    'Maurten DM',
  sis:        'SiS Beta Fuel',
  precision:  'Precision PF',
};
// Brands with partial dual-transport SKUs (some products yes, others no)
const PARTIAL_DUAL_BRANDS = {
  gu:        'GU Roctane',
  tailwind:  'Tailwind Endurance Fuel',
};

function listBrandNames(brandKeys, brandMap) {
  const matched = (brandKeys ?? []).filter(b => brandMap[b]).map(b => brandMap[b]);
  if (matched.length === 0) return null;
  if (matched.length === 1) return matched[0];
  if (matched.length === 2) return `${matched[0]} and ${matched[1]}`;
  return `${matched.slice(0, -1).join(', ')}, and ${matched[matched.length - 1]}`;
}

// Build the ramp trajectory (e.g. [62, 72, 80]) given current g/hr, target, weeks available.
// Returns null if data missing or already at target.
function buildRampSchedule(currentGhr, targetGhr, weeksOut) {
  if (currentGhr == null || targetGhr == null || weeksOut == null || weeksOut < 1) return null;
  if (currentGhr >= targetGhr) return null;
  const steps = [];
  let cur = currentGhr;
  let week = 0;
  while (cur < targetGhr && week < 4) {
    cur = Math.min(targetGhr, cur + 10);
    steps.push(cur);
    week++;
  }
  return steps;
...
// === Tactic generators — return personalized tactic arrays from athlete context ===
// Context shape: { athleteGhr, targetGhr, weeksOut, brands, weeklyHours }
const TACTIC_GENERATORS = {
  carb_deficit: (ctx) => {
    const { athleteGhr, targetGhr, weeksOut, brands } = ctx;
    const ramp = buildRampSchedule(athleteGhr, targetGhr, weeksOut);
    const rampLine = ramp
      ? `Go ${athleteGhr} → ${ramp.join(' → ')} g/hr over the next ${ramp.length} ${pluralWeeks(ramp.length)}` +
        (weeksOut > ramp.length
          ? `, then consolidate at ${targetGhr} g/hr for the remaining ${weeksOut - ramp.length} ${pluralWeeks(weeksOut - ramp.length)} before race-week taper. `
          : `. `) +
        `SGLT1 carb transporters physically upregulate with structured exposure — the schedule matters more than the peak.`
      : `Increase intake by 10 g/hr each week until You hit the trained-gut range. SGLT1 carb transporters physically upregulate with structured exposure — the schedule matters more than the peak.`;

    const dualBrands    = listBrandNames(brands, DUAL_TRANSPORT_BRANDS);
    const partialBrands = listBrandNames(brands, PARTIAL_DUAL_BRANDS);
    const dualLine = dualBrands
      ? `${dualBrands} ${dualBrands.includes(' and ') || dualBrands.includes(',') ? 'are' : 'is'} already 1:0.8 dual-transport — the format works above 60 g/hr. Your gap is dose, not type. Increase how much, not what.`
      : partialBrands
      ? `${partialBrands} ${partialBrands.includes(' and ') || partialBrands.includes(',') ? 'are' : 'is'} partially dual-transport. Fine for ramping, but for race-day reliability above 70 g/hr add a true 1:0.8 product (Maurten DM, SiS Beta Fuel, or Precision PF).`
      : `Single-source glucose maxes at ~60 g/hr regardless of effort. To push absorption above 60, switch to a dual-transport (1:0.8 glucose:fructose) product like Maurten DM, SiS Beta Fuel, or Precision PF.`;
    ...
    return [
      ...
      { title: 'Ramp gut training in steps',
        body: rampLine },
  ...
  gi_distress: (ctx) => {
    const { weeksOut } = ctx;
    const startLine = (weeksOut != null && weeksOut >= 4)
      ? `Start at 30 g/hr on Your next race-pace session. If it holds, push to 40, then 50, building one step per week. You have ${weeksOut} ${pluralWeeks(weeksOut)} — enough for a controlled ramp.`
      : (weeksOut != null && weeksOut >= 2)
      ? `Start at 30 g/hr on Your next race-pace session. With ${weeksOut} ${pluralWeeks(weeksOut)} until race, focus on tolerance over intake peaks — don't try to maximize this close.`
      : `Start at 30 g/hr on Your next race-pace session. If it holds, push to 40 g/hr the following week. The ramp matters more than the peak.`;
...
export function identifyPrimaryGap(intake) {
  ...
  // Context passed to every tactic generator — single source of personalization data
  const tacticCtx = {
    athleteGhr,
    targetGhr,
    weeksOut,
    brands: intake.brands ?? [],
  ...
  if (!winner) {
    return {
      ...
      tactics: TACTIC_GENERATORS.recovery(tacticCtx),
  ...
  return {
    ...
    tactics: TACTIC_GENERATORS[winner.type](tacticCtx),
    routing: chooseRouting({ raceDistance, weeksOut }),
    athleteGhr,
    targetGhr,
    weeksOut,
...
export { TACTIC_GENERATORS, GAP_TITLES, RACE_TARGET_GHR };
